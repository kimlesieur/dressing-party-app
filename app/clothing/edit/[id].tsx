      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={styles.textInput} value={clothingData.name} onChangeText={text => setClothingData(p => p ? {...p, name: text} : null)} />
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.inputLabel}>Type *</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.optionsScroll}>
              {clothingTypes.map(type => (
                <TouchableOpacity key={type.id} style={[styles.optionChip, clothingData.type === type.id && styles.selectedChip]} onPress={() => setClothingData(p => p ? { ...p, type: type.id, subCategory: '' } : null)}>
                  <Text style={[styles.optionText, clothingData.type === type.id && styles.selectedText]}>{type.name}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.optionsScroll}>
                {selectedType.subCategories.map(subCat => (
                  <TouchableOpacity key={subCat} style={[styles.optionChip, clothingData.subCategory === subCat && styles.selectedChip]} onPress={() => setClothingData(p => p ? { ...p, subCategory: subCat } : null)}>
                    <Text style={[styles.optionText, clothingData.subCategory === subCat && styles.selectedText]}>{subCat}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            </View>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={styles.textInput} value={clothingData.brand} onChangeText={text => setClothingData(p => p ? {...p, brand: text} : null)} />
          </View>
        </View>
        ...
        <View style={styles.section}>
            ...
            <View style={styles.optionsContainer}>
                {seasons.map(s => (
                    <TouchableOpacity key={s} style={[styles.optionChip, clothingData.seasons.includes(s) && styles.selectedChip]} onPress={() => handleToggle('seasons', s)}>
                        <Text style={[styles.optionText, clothingData.seasons.includes(s) && styles.selectedText]}>{s}</Text>
                    </TouchableOpacity>
                ))}
            </View>
        </View>
        <View style={styles.section}>
            ...
            <View style={styles.optionsContainer}>
                {colors.map(c => (
                    <TouchableOpacity key={c} style={[styles.optionChip, clothingData.colors.includes(c) && styles.selectedChip]} onPress={() => handleToggle('colors', c)}>
                        <Text style={[styles.optionText, clothingData.colors.includes(c) && styles.selectedText]}>{c}</Text>
                    </TouchableOpacity>
                ))}
            </View>
        </View>
        <View style={styles.section}>
            ...
            <TextInput style={styles.textArea} multiline value={clothingData.notes} onChangeText={text => setClothingData(p => p ? {...p, notes: text} : null)} />
        </View>
...
const styles = StyleSheet.create({
    ...
    removeImageButton: { position: 'absolute', top: 10, right: 10, backgroundColor: 'rgba(0,0,0,0.6)', padding: 5, borderRadius: 15 },
    imageUploadContainer: { flexDirection: 'row', justifyContent: 'space-around' },
    imageUploadButton: { alignItems: 'center', backgroundColor: 'white', padding: 20, borderRadius: 12, flex: 1, marginHorizontal: 5 },
    imageUploadText: { marginTop: 10, color: '#8B5CF6' },
    inputGroup: { marginBottom: 15 },
    inputLabel: { fontSize: 16, color: '#374151', marginBottom: 8 },
    textInput: { backgroundColor: 'white', padding: 15, borderRadius: 10, fontSize: 16 },
    textArea: { backgroundColor: 'white', padding: 15, borderRadius: 10, fontSize: 16, height: 100, textAlignVertical: 'top' },
    optionsScroll: { marginHorizontal: -20, paddingHorizontal: 20 },
    optionsContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
    optionChip: { backgroundColor: 'white', paddingVertical: 10, paddingHorizontal: 15, borderRadius: 20, borderWidth: 1, borderColor: '#E5E7EB' },
    selectedChip: { backgroundColor: '#8B5CF6', borderColor: '#8B5CF6' },
    optionText: { color: '#374151' },
    selectedText: { color: 'white' },
    saveButton: { backgroundColor: '#8B5CF6', padding: 20, borderRadius: 15, alignItems: 'center', margin: 20 },
